		...
		new Vue({
		    ...
		    data: {
		    	page: {
		    		...
		    		back: '<?php echo base_url()."my-class/class-selected/".$classID ?>'
		    	},
		    	show_modal: false,
		    	e_date: '',
		    ...
		    methods: {
		    	fetch_stud_data_in_class(){
		    		this.$http.get('<?php echo base_url() ?>my_class/fetch_stud_data_in_class/'+this.classID+'/'+this.studID)
		            .then(response => {
		    	...
		    	save(){
		    		...
		    		if(this.validate(g)){
		    			...
		    			this.$http.post('<?php echo base_url() ?>my_class/save_grade',data)
			            .then(response => {
			            	const c = response.body
		    	...
		    	submit_action(){
		    		...
		    		this.$http.post('<?php echo base_url() ?>my_class/drop_or_inc',data)
		            .then(response => {
		    	...
		    	comply(){
		    		...
		    		this.$http.post('<?php echo base_url() ?>my_class/comply',data)
		            .then(response => {
		    	...
		    	comply2(){
		    		...
		    		this.$http.post('<?php echo base_url() ?>my_class/comply2',data)
		            .then(response => {
		            	console.log(response.body)
		            	const c = response.body
		            	if(c.status == 'error'){
		    	...
		    	is_safe_to_remove(){
		    		this.$http.get('<?php echo base_url() ?>my_class/is_safe_to_remove/'+this.classID+'/'+this.studID)
		            .then(response => {
		            	const c = response.body
		            	if(c == 'yes'){
		            		swal({
		    	...
		    	remove_stud_from_class(){
		    		this.$http.get('<?php echo base_url() ?>my_class/remove_stud_from_class/'+this.classID+'/'+this.studID)
		            .then(response => {
		            	swal(this.student_name+' successfully removed in this class!', {icon: 'success'})
		            	.then((ok) => {
						  if(ok){
						  	window.location.href='<?php echo base_url() ?>my-class/class-selected/'+this.classID
						  }
						});
		            })
		    	}
