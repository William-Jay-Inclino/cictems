	...
	<div class="container" style="max-width: 600px;">
		...
		<div class="box">
			...
			<table class="table is-fullwidth">
				...
				<tr>
					<td width="50%">Enrollment Confirmation</td>
					<td width="50%">
						<div class="field">
						  <input id="switchConf" type="checkbox" name="switchNormal" class="switch is-rounded" @change="updateAccess(25)" <?php if(in_array("25", $record['modules'])){echo 'checked="checked"';} ?> >
						  <label for="switchConf"></label>
						</div>
					</td>
				</tr>
			</table>
...
<script>
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		new Vue({
		    ...
		    methods: {
		    	updateAccess(val){
		    		this.$http.post('<?php echo base_url() ?>users_staff/updateAccess', {uID: this.uID, modID: val})
		        	.then(response => {
		        		if(response.body != ''){
		        			alert('Oooops! Something wrong happen')
		        			window.location.href = '<?php echo base_url()."users/faculty/access-rights/".$record['staffID'] ?>'
		        		}
					 })
		    	}
		    },
