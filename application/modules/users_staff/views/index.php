			...
			<table class="table is-fullwidth">
				...
				<tbody v-show="!loading">
					<tr v-for="record, i in records">
						...
						<td>
							<a :href="page.show + '/' + record.staffID" class="button is-outlined is-primary"><i class="fa fa-angle-double-right fa-lg"></i></a>
						</td>
					</tr>
				</tbody>
			</table>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	new Vue({
	    ...
	    data: {
	    	...
	    	page:{
		    	data_list: 'Staff list',
		    	add: 'Add Staff',
		    	add_link: '<?php echo base_url() ?>users/staff/form',
		    	show: '<?php echo base_url() ?>users/staff/show'
	    	},
	    ...
	    methods: {
	    	changeStatus(i){
		    	...
		    	this.$http.post('<?php echo base_url() ?>users_staff/changeStatus', {uID: record.uID,status: new_stat})
	        	.then(response => {
	        ...
	        fetchData(page) {
	        	...
	        	this.loading = true
	        	this.msg = false
	        	this.$http.get('<?php echo base_url() ?>users_staff/read/'+this.option+'/'+this.value + '/' + page + '/' + this.per_page)
	        	.then(response => {
