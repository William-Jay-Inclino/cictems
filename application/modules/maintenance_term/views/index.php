			...
			<table class="table is-fullwidth is-centered">
				...
				<tbody v-show="!loading">
					<tr v-for="record, i in records">
						...
						<td>{{record.semDesc}}</td>
						<td>
							<span v-if="record.termStat == 'active'">
						...
						<td>
							<a :href="page.show + '/' + record.termID" class="button is-outlined is-primary"><i class="fa fa-angle-double-right fa-lg"></i></a>
						</td>
					</tr>
				</tbody>
			</table>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	new Vue({
	    ...
	    data: {
	    	...
	    	pagination: true,
	    	msg: false,
	    	current_term: '<?php echo $current_term->termID; ?>',

	    	page:{
		    	data_list: 'Term list',
		    	add: 'Add Term',
		    	add_link: '<?php echo base_url() ?>maintenance/term/form',
		    	show: '<?php echo base_url() ?>maintenance/term/show'
	    	},
	    ...
	    methods: {
	        ...
	        fetchData(page) {
	        	...
	        	this.loading = true
	        	this.msg = false
	        	this.$http.get('<?php echo base_url() ?>maintenance_term/read/'+this.value + '/' + page + '/' + this.per_page)
	        	.then(response => {
