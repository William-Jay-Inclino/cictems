...
document.addEventListener('DOMContentLoaded', function() {
	...
	new Vue({
	    ...
	    data: {
	    	...
	    	page:{
		    	data_list: 'Student list',
		    	add: 'Add Student',
		    	add_link: '<?php echo base_url() ?>users/student/form',
		    	show: '<?php echo base_url() ?>users/student/show'
	    	},
    		...
    		option: 's.controlNo',
    		search_options: [
    			{value: 's.controlNo', text: 'ControlNo'},
	    ...
	    methods: {
	    	changeStatus(i){
		    	...
		    	this.$http.post('<?php echo base_url() ?>users_student/changeStatus', {uID: record.uID,status: new_stat})
	        	.then(response => {
	        ...
	        fetchData(page) {
	        	...
	        	this.loading = true
	        	this.msg = false
	        	this.$http.get('<?php echo base_url() ?>users_student/read/'+this.option+'/'+this.value + '/' + page + '/' + this.per_page)
	        	.then(response => {
