			...
			<table class="table is-fullwidth is-centered">
				...
				<tbody v-show="!loading">
					<tr v-for="record, i in records">
						...
						<td>{{record.courseDesc}}</td>
						<td>
							<a :href="page.show + '/' + record.courseID" class="button is-outlined is-primary"><i class="fa fa-angle-double-right fa-lg"></i></a>
						</td>
					</tr>
				</tbody>
			</table>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	new Vue({
	    ...
	    data: {
	    	...
	    	page:{
		    	data_list: 'Course list',
		    	add: 'Add Course',
		    	add_link: '<?php echo base_url() ?>maintenance/course/form',
		    	show: '<?php echo base_url() ?>maintenance/course/show'
	    	},
    		...
    		option: 'courseCode',
    		search_options: [
    			{value: 'courseCode', text: 'Code'},
    			{value: 'courseDesc', text: 'Description'}
    		],
	    ...
	    methods: {
	        ...
	        fetchData(page) {
	        	...
	        	this.loading = true
	        	this.msg = false
	        	this.$http.get('<?php echo base_url() ?>maintenance_course/read/'+this.option+'/'+this.value + '/' + page + '/' + this.per_page)
	        	.then(response => {
