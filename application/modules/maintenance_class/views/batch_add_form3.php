			...
			<table class="table is-fullwidth is-centered">
				...
				<tbody>
					<tr v-for="record, i in classes" :class="{err: record.error}">
						...
						<td>
							<button class="button btn-width" @click="changeStatus2(record.status2, i)">
							...
							<button class="button btn-width" @click="checkConflict(i)">
								<i class="fa fa-info has-text-link"></i>
							</button>
						</td>
					</tr>
				</tbody>
	...
	new Vue({
	    ...
	    methods: {
	        ...
	        checkConflict(i){
	        	...
	        	if(c[i].status2 == 0){
	        		if(sec == null){
	        			swal('Section is required','',{icon: 'warning'})
	        		}else if(!c[i].timeIn || !c[i].timeOut || c[i].room == null || c[i].day == null || c[i].faculty == null){
		        		c[i].error = true
		        		swal('Fill up the fields first','',{icon: 'warning'})
		        	}else{
		        		this.$http.post('<?php echo base_url() ?>maintenance_class/checkConflict', {class: c[i], secID: sec.secID})
			        	.then(response => {
			        		console.log(response.body)
						 })
		        	}
	        	}else{
	        		swal('Class is not offered','','warning')
	        	}
	        }
