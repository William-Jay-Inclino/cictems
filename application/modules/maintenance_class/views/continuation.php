			...
			<table v-for="record, i in classes" class="table is-fullwidth is-centered">
				<tbody>
					<tr :class="{err: record.error}">
						...
						<td class="row-10">
							<multiselect @input="checkConflict(i)" v-model="record.day" track-by="dayID" label="dayDesc" :options="days" placeholder="" :allow-empty="false" :disabled="record.status2 == 1"></multiselect>
						</td>
						<td class="row-13">
							<multiselect @input="checkConflict(i)" v-model="record.room" track-by="roomID" label="roomName" :options="rooms" placeholder="" :allow-empty="false" :disabled="record.status2 == 1"></multiselect>
						</td>
						<td class="row-19">
							<multiselect @input="checkConflict(i)" v-model="record.faculty" track-by="facID" label="faculty" :options="faculties" placeholder="" :allow-empty="false" :disabled="record.status2 == 1"></multiselect>
						</td>
						<td>
	    ...
	    methods: {
	    	...
	    	checkConflict3(i, c){
	    		...
	    		for(let [ii, cc] of classes.entries()){
    				...
    				if(cc.timeIn && cc.timeOut != null && cc.day != null && cc.room != null && cc.faculty != null && cc.status2 == 0 && !cc.unit_conflict){
