			...
			<table class="table is-fullwidth is-centered">
				...
				<tbody v-show="!loading">
					<tr v-for="record, i in records">
						<td>{{record.secName}}</td>
						<td>{{record.courseCode}}</td>
						<td>{{record.yearDesc}}</td>
						<td>{{record.semDesc}}</td>
						<td>
							<a :href="page.show + '/' + record.secID" class="button is-outlined is-primary"><i class="fa fa-angle-double-right fa-lg"></i></a>
						</td>
					</tr>
				</tbody>
			</table>
	...
	new Vue({
	    ...
	    data: {
	    	...
	    	page:{
		    	...
		    	add: 'Add Section',
		    	add_link: '<?php echo base_url() ?>maintenance/section/form',
		    	show: '<?php echo base_url() ?>maintenance/section/show'
	    	},
	    ...
	    methods: {
	        ...
	        fetchData(page) {
	        	...
	        	this.loading = true
	        	this.msg = false
	        	this.$http.get('<?php echo base_url() ?>maintenance_section/read/'+this.option+'/'+this.value + '/' + page + '/' + this.per_page)
	        	.then(response => {
