...
class Maintenance_Prospectus extends MY_Controller{

	function __construct(){
		parent::__construct(8);
		$this->_data['module'] = 'maintenance_prospectus';
	...
	function form($id = NULL){
		if($id == NULL){
			...
			$this->mdl_prospectus->check_form_id($id[0]);
			$this->_data['record'] = $this->mdl_prospectus->read_one($id[0]);
	...
	function create(){
		$this->mdl_prospectus->create();
	}

	function read($data){ 
		$this->mdl_prospectus->read($data[0], $data[1], $data[2], $data[3]);
	}

	function update(){ //parameter is an array since _remap method is used
		$this->mdl_prospectus->update();
	}

	function delete($id){ //parameter is an array since _remap method is used
		$this->mdl_prospectus->delete($id[0]);
	}

	function is_safe_delete($id){
		$this->mdl_prospectus->is_safe_delete($id[0]);
	}

	function get_courses(){
		$this->mdl_prospectus->get_courses();	
	}
